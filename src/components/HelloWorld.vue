<template>
  <div v-if="easyFlowVisible" style="height:100vh;display:flex;background-color:#fff;">
    <div id="efContainer" ref="efContainer" v-flowDrag @click='cancelNode'>
      <template v-for="node in data.nodeList">
        <flow-node :id="node.id" :key="node.id" :node="node" :activeElement="activeElement" @changeNodeSite="changeNodeSite" @nodeRightMenu="nodeRightMenu" @clickNode="clickNode">
        </flow-node>
      </template>
      <!-- 给画布一个默认的宽度和高度 -->
    ...
    <!-- 右侧表单 -->
    <div style="width: 300px;border-left: 1px solid #dce3e8;background-color: #fff;height:100%">
      <div style="height:30%">
...
export default {
  ...
  methods: {
    ...
    cancelNode() {
      ...
      this.$refs.nodeForm.nodeRefresh()
    }
  }
}
</script>
